...
#tuned_model_Neuralnetwork = TunedModel(model = model_Neuralnetwork, resampling= CV(nfolds = 10), measure = auc, range = [range(model_Neuralnetwork, :(epochs), values = [5, 10]), range(model_Neuralnetwork, :lambda, lower = 2e-1 , upper = 2, scale = :log), range(model_Neuralnetwork, :batch_size, values = [20,40]), range(model_Neuralnetwork, :alpha, values = [0,0.5] )])#, acceleration=CUDALibs()) #, tune: optimiser, 
#s4 
...
MLJ.save("mach_Neuralnetwork_tuned_s5.jlso", mach_Neuralnetwork_tuned)
...
write_csv("neural_newtork_s5.csv", prediction_Neuralnetwork_df)
