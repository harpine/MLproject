...
# model_not_regul = LogisticClassifier(penalty = :none)
# mach_mult_logistic_not_reg = fit!(machine(model_not_regul, training_dropped_x,
# training_dropped_y))
...
#CSV.write(joinpath(output_folder, "multi_logistic_reg_not_tuned.csv"), prediction_mult_logistic_not_tuned_l2_df)
...
#                                    tuning = Grid(),
#                                    range = range(model_mult_logistic_reg_l1, :lambda, lower = 2e-2 , upper = 2e-1, scale = :log),
...
#                                    resampling = CV(nfolds = 10),
#                                    tuning = Grid(),
#                                    range = range(model_mult_logistic_reg_l1, :lambda, lower = 2e-30 , upper = 2e-10, scale = :log),
